async function buscarGeralP() {
    await fetch("http://localhost:7000/api/Pedido/listarPedidos")
...
async function carregarClientes() {
    ...
    let resposta = await fetch("http://localhost:7000/api/Cliente");
...
async function cadastrarpedido() {
    ...
    const idcliente = document.getElementById("clientepedido").value;
    ...
    await fetch("http://localhost:7000/api/Pedido", {
        method: "POST",
        headers: {
...
async function atualizarpedido() {
    ...
    try {
        await fetch(`http://localhost:7000/api/Pedido/${pedidoeditado}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deletar(id){
     ...
     if(!confirmar){
      ...
      await fetch(`http://localhost:7000/api/Pedido/${id}`,{
            method:'DELETE'
